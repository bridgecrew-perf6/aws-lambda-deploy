...
```bash
ns-lambda-codedeploy$ sam build
```
...
```bash
ns-lambda-codedeploy$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
ns-lambda-codedeploy$ sam local start-api
ns-lambda-codedeploy$ curl http://localhost:3000/
```
...
```bash
ns-lambda-codedeploy$ sam logs -n HelloWorldFunction --stack-name ns-lambda-codedeploy --tail
```
...
```bash
ns-lambda-codedeploy$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name ns-lambda-codedeploy
```
